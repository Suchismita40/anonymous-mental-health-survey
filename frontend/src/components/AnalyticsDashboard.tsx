      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-800/80 pb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={onRefresh}
            disabled={isLoading}
            className="p-2 text-slate-400 hover:text-purple-300 bg-slate-900 hover:bg-slate-800 border border-slate-800 rounded-lg transition-all flex items-center justify-center"
            title="Refresh Analytics"
          >
            <svg width={16} height={16} className={`w-4 h-4 ${isLoading ? 'animate-spin text-purple-400' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
